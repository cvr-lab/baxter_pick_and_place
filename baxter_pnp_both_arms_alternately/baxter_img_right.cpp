...
void ImageCallBack(const sensor_msgs::Image &msg)
{
  ...
  cvShowImage("detected objects right", src_result);
  //cvSaveImage("/home/maik/Schreibtisch/save/detected objects right.jpg", src);
  for(int i = 0; i != NUM_OBJ; i++)
  {
    // Assign values to pose and add offsets (in cm) to get the position
...
{  
  ...
  ros::NodeHandle n;
  pos_pub = n.advertise<geometry_msgs::PoseArray>("detected_objects_right", 2);
