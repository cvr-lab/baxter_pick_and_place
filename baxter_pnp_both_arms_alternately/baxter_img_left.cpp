...
void ImageCallBack(const sensor_msgs::Image &msg)
{
  ...
  for(int i = 0; i != NUM_OBJ; i++)
  {
    ...
    pose.position.x = rel_pos[i].x / 1000+0.07+0.57-0.02723+0.01;
    pose.position.y = rel_pos[i].y / 1000-0.032+0.03245+0.402;
  ...
  cvShowImage("detected objects left", src_result);
  //cvSaveImage("/home/maik/Schreibtisch/save/detected objects left.jpg", src);
  for(int i = 0; i != NUM_OBJ; i++)
  {
    // Assign values to pose and add offsets (in cm) to get the position 
...
{  
  ...
  ros::NodeHandle n;
  pos_pub = n.advertise<geometry_msgs::PoseArray>("detected_objects_left", 2);
  ros::Subscriber sub = n.subscribe("/cameras/left_hand_camera/image", 1, ImageCallBack);
  ros::spin();
  return 0;
}
